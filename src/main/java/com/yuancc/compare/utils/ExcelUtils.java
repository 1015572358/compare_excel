package com.yuancc.compare.utils;
...
public class ExcelUtils {
    ...
    public static <T> LinkedList<T> readExcel(Class<T> cls, File file, int startrow){
        log.info("开始读取xlsx数据");
                        ...
                        if (firstRow) {
                            for (int j = row.getFirstCellNum(); j <= row.getLastCellNum(); j++) {
                                ...
                                if (isMergedRegion(sheet, i, j)) {
                                    cellValue = isCombineCell(combineCell, cell, sheet);
                                }
                            ...
                            try {
                                ...
                                if (!allBlank) {
//                                    Field rownum = t.getClass().getDeclaredField("rownum");
//                                    rownum.setAccessible(true);
//                                    rownum.set(t, (i + 1) + "");//记录行数
                                    dataList.add(t);
                                } else {
...
//            Date date = new SimpleDateFormat("EEE MMM dd HH:mm:ss Z yyyy", Locale.UK).parse(value);
    ...
    private static <T> void handleField2(T t, String value, String name) throws Exception {
        ...
        if (type == Object.class) {
            ...
            field.set(t, BooleanUtils.toBoolean(value));
        } else if (type == Date.class && value!=null && !"".equals(value)) {
            Date date = new SimpleDateFormat("EEE MMM dd HH:mm:ss Z yyyy", Locale.UK).parse(value);
            //转换日期
            field.set(t, date);
        } else if (type == String.class) {
    ...
    private static String getCellValue(Cell cell) {
        ...
        if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
            ...
            return StringUtils.trimToEmpty(cell.getStringCellValue().replace(" ",""));
    ...
    public static <T> void writeExcel(HttpServletResponse response, List<T> dataList, Class<T> cls){
        ...
        //浏览器下载excel
        buildExcelDocument("网格通行证.xlsx",wb,response);
        //生成excel文件
//        buildExcelFile(".\\default.xlsx",wb);
    ...
    public static <T> boolean checkFields(T t) throws Exception{
        Field[] fs=t.getClass().getDeclaredFields();
        List<String> list=new ArrayList<String>();
        for(Field field:fs){
            field.setAccessible(true);
            if(field.get(t)!=null&&field.get(t)!=""&&!"null".equalsIgnoreCase(field.get(t).toString()) && !field.getName().equals("serialVersionUID")){
                return true;
            }
        }
        return false;
